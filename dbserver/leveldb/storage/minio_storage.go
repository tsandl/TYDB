...
import (
	...
	"errors"
	"fmt"
	"github.com/tsandl/TYDB/dbserver/miomanager"
	"io"
	"io/ioutil"
	"os"
	"path/filepath"
...
var (
	errFileOpenMinIO = errors.New("leveldb/storage: MinIO still open")
	errReadOnlyMinIO = errors.New("leveldb/storage: MinIO storage is read-only")
)

type MinioLock interface {
	release() error
}

type MinioStorageLock struct {
	fs *fileStorage
}

//func (lock *MinioStorageLock) UnlockMinio() {
//	if lock.fs != nil {
//		lock.fs.mu.Lock()
...
type int64Slice1 []int64

func (p int64Slice) Len1() int           { return len(p) }
func (p int64Slice) Less1(i, j int) bool { return p[i] < p[j] }
func (p int64Slice) Swap1(i, j int)      { p[i], p[j] = p[j], p[i] }

func writeMinioSynced(bucketName string, filename string, data []byte, perm os.FileMode) error {
	...
	//file, filestat := minio.readFile1(filePath)
...
const minioLogSizeThreshold = 1024 * 1024 // 1 MiB

// minioStorage is a file-system backed storage.
...
func OpenMinio(path string, readOnly bool) (Storage, error) {
...
func minioGenName(fd FileDesc) string {
	switch fd.Type {
	case TypeManifest:
...
func minioHasOldName(fd FileDesc) bool {
	return fd.Type == TypeTable
}

func minioGenOldName(fd FileDesc) string {
	switch fd.Type {
	case TypeTable:
...
func minioParseName(name string) (fd FileDesc, ok bool) {
	var tail string
...
func minioParseNamePtr(name string, fd *FileDesc) bool {
